...
use syn::visit::Visit;
use syn::{Attribute, BinOp, Expr, File, ItemFn, ReturnType, UnOp};
...
struct Function<'r> {
    /// Most of the information about the function. In particular, everything we
    /// need to return record and display the mutant.
    function: Arc<mutate::Function>,
    /// The functions return type, tracked independently because we we want
    /// `mutate::Function` to be `Send`.
...
impl<'o, 'r> DiscoveryVisitor<'o, 'r> {
    fn enter_function(
        ...
        let function = Arc::new(mutate::Function {
    ...
    fn collect_fn_mutants(&mut self, block: &impl ToTokens) {
        if let Some(function) = self.fn_stack.last() {
            ...
            if repls.is_empty() {
                debug!(
                    function_name = function.function.function_name,
...
impl<'ast> Visit<'ast> for DiscoveryVisitor<'_, 'ast> {
    ...
    // Visit `return ...` to rewrite explicit returns.
