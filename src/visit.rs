...
use syn::visit::Visit;
use syn::{Attribute, BinOp, Expr, File, ItemFn, ReturnType, Stmt, UnOp};
...
struct Function<'r> {
    function: Arc<mutate::Function>,
    /// Tracked independently from the string repr of the return type in
    /// `function` because we want `mutate::Function` to be `Send`.
    return_type: &'r ReturnType,
}

impl<'o, 'r> DiscoveryVisitor<'o, 'r> {
    fn enter_function(
        &mut self,
        function_name: &Ident,
        return_type: &'r ReturnType,
        ...
        let function = Arc::new(mutate::Function::function(
            full_function_name,
            return_type,
            span,
        ));
    ...
    fn leave_function(&mut self, function: Arc<mutate::Function>) {
        self.namespace_stack
            .pop()
            .expect("Namespace stack should not be empty");
        assert_eq!(
            self.fn_stack.pop().map(|f| f.function),
            Some(function),
    ...
    fn enter_closure(
        ...
        let function = Arc::new(mutate::Function::closure(&return_type, span));
        self.fn_stack.push(Function {
            function: function.clone(),
            return_type,
        });
    ...
    fn leave_closure(&mut self, function: Arc<mutate::Function>) {
        assert_eq!(
            self.fn_stack.pop().map(|f| f.function),
            Some(function),
            "Function stack mismatch"
    ...
    fn collect_mutant(&mut self, span: Span, replacement: TokenStream, genre: Genre) {
        self.mutants.push(Mutant {
            ...
            function: self.fn_stack.last().map(|f| f.function.clone()),
            span,
    ...
    fn collect_fn_mutants(&mut self, block: &impl ToTokens) {
        if let Some(function) = self.fn_stack.last() {
            ...
            if repls.is_empty() {
                debug!(
                    function_name = function.function.function_name_str(),
                    return_type = function.function.return_type,
                    "No mutants generated for this return type"
                );
            } else {
                let orig_block = tokens.to_pretty_string();
                for rep in repls {
                    ...
                    let new_block = rep.to_token_stream().to_pretty_string();
                    // dbg!(&orig_block, &new_block);
                    if orig_block == new_block {
                        debug!("Replacement is the same as the function body; skipping");
                    } else {
                        self.collect_mutant(tokens.span().into(), quote! { #rep }, Genre::FnValue);
                    }
                }
            }
        } else {
...
impl<'ast> Visit<'ast> for DiscoveryVisitor<'_, 'ast> {
    /// Visit a source file.
    ...
    fn visit_item_fn(&mut self, i: &'ast ItemFn) {
        ...
        let mut function_return_rewriter = FunctionReturnRewriteVisitor { parent: self };
        function_return_rewriter.visit_block(&i.block);
    ...
    fn visit_trait_item_fn(&mut self, i: &'ast syn::TraitItemFn) {
        ...
        if let Some(block) = &i.default {
            ...
            let function = self.enter_function(&i.sig.ident, &i.sig.output, i.span());
            let mut function_return_rewriter = FunctionReturnRewriteVisitor { parent: self };
            function_return_rewriter.visit_block(block);
    ...
    fn visit_expr_closure(&mut self, i: &'ast syn::ExprClosure) {
        let closure = self.enter_closure(&i.output, i.span());
        let mut function_return_rewriter = FunctionReturnRewriteVisitor { parent: self };
        function_return_rewriter.visit_expr(&i.body);
        syn::visit::visit_expr_closure(self, i);
        self.leave_closure(closure);
    }

    fn visit_expr_return(&mut self, i: &'ast syn::ExprReturn) {
        self.collect_fn_mutants(&i.expr);
    ...
    fn visit_stmt(&mut self, i: &'ast Stmt) {
        ...
        // returns should be viable, and Rust style is to not explicit return
    ...
    fn visit_expr_method_call(&mut self, i: &'ast syn::ExprMethodCall) {
        match i.method.to_string().as_str() {
            ...
            | "step_by" | "take" => {
                self.collect_mutant(
                    Span {
                        start: i.dot_token.span().start().into(),
                        end: i.span().end().into(),
                    },
                    quote! {},
                    Genre::Statement,
            ...
            // These functions are `Option<T> -> T` which is also implemented by
            // a plain `unwrap`.
...
struct FunctionReturnRewriteVisitor<'p, 'o, 'r> {
    parent: &'p mut DiscoveryVisitor<'o, 'r>,
}

impl<'ast> Visit<'ast> for FunctionReturnRewriteVisitor<'_, '_, 'ast> {
    /// Don't recurse into other function definitions.
    fn visit_item_fn(&mut self, _i: &'ast syn::ItemFn) {}

    fn visit_block(&mut self, i: &'ast syn::Block) {
        // Only visit last statement
        if let Some(last_stmt) = i.stmts.last() {
            self.visit_stmt(last_stmt);
        }
    }

    fn visit_expr_if(&mut self, i: &'ast syn::ExprIf) {
        // Don't visit the condition.
    ...
    fn visit_expr(&mut self, i: &'ast syn::Expr) {
        match i {
            // `Paren` doesn't need a special `visit` function, we just want to
            // recurse like normal without rewriting the whole expr.
            Expr::Paren(_) |
            // These are handled by the `visit` functions above, typically
            // skipping over some sub-exprs we don't want to to rewrite.
            Expr::Block(_) | Expr::If(_) | Expr::Match(_) => syn::visit::visit_expr(self, i),
            // Everything else is clobbered, replaced by a new expression
            // according to `return_type_replacements`.
            _ => self.parent.collect_fn_mutants(i),
        }
    }
}
